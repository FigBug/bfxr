...
public:
    ...
    std::vector<std::string> _lockedParams; // stores list of strings, these strings represent parameters that will be locked during randomization/mutation
    ...
    void setParamLocked(std::string param, bool locked)
    {
        ...
        if (locked)
        {
            if (itr == _lockedParams.end())
            {
                _lockedParams.push_back(param);
                paramsDirty = true;
            }
        }
        else
    ...
    void resetParams()
    {
        ...
        _lockedParams.clear();
        _lockedParams.push_back("masterVolume");
    }
